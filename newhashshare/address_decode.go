...
import (
	"fmt"
	"github.com/assetsadapterstore/newhashshare-adapter/newhashshare_addrdec"
...
func (decoder *addressDecoder) PrivateKeyToWIF(priv []byte, isTestnet bool) (string, error) {
	...
	cfg := newhashshare_addrdec.NHSS_mainnetPrivateWIFCompressed
	if decoder.wm.Config.IsTestNet {
		cfg = newhashshare_addrdec.NHSS_testnetPrivateWIFCompressed
	}

	wif, _ := newhashshare_addrdec.Default.AddressEncode(priv, cfg)
...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	newhashshare_addrdec.Default.IsTestNet = decoder.wm.Config.IsTestNet
	address, err := newhashshare_addrdec.Default.AddressEncode(pub)
	if err != nil {
		return "", err
	}
...
func (decoder *addressDecoder) RedeemScriptToAddress(pubs [][]byte, required uint64, isTestnet bool) (string, error) {
	...
	cfg := newhashshare_addrdec.NHSS_mainnetAddressP2SH
	if decoder.wm.Config.IsTestNet {
		cfg = newhashshare_addrdec.NHSS_testnetAddressP2SH
	}
	...
	address, _ := newhashshare_addrdec.Default.AddressEncode(pkHash, cfg)
...
func (decoder *addressDecoder) WIFToPrivateKey(wif string, isTestnet bool) ([]byte, error) {
	...
	priv, err := newhashshare_addrdec.Default.AddressDecode(wif, cfg)
	if err != nil {
		return nil, err
	}
