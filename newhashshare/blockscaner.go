...
type NHSSBlockScanner struct {
	*bitcoin.BTCBlockScanner
	nhssWM *WalletManager
}

//NewBTCBlockScanner 创建区块链扫描器
func NewNHSSBlockScanner(wm *WalletManager) *NHSSBlockScanner {
	bs := &NHSSBlockScanner{nhssWM: wm}
...
func (bs *NHSSBlockScanner) GetBalanceByAddress(address ...string) ([]*openwallet.Balance, error) {

	return bs.nhssWM.getBalanceCalUnspent(address...)
