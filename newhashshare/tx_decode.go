...
func (decoder *TransactionDecoder) SignBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			txHash := nhssTransaction.TxHash{
				Hash: keySignature.Message,
				Normal: &nhssTransaction.NormalTx{
					Address: keySignature.Address.Address,
					SigType: nhssTransaction.SigHashAll,
				},
			}
			...
			//签名交易
			/////////交易单哈希签名
			sigPub, err := nhssTransaction.SignRawTransactionHash(txHash.GetTxHashHex(), keyBytes)
			if err != nil {
...
func (decoder *TransactionDecoder) VerifyBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	var (
		txUnlocks  = make([]nhssTransaction.TxUnlock, 0)
		emptyTrans = rawTx.RawHex
		//sigPub     = make([]nhssTransaction.SignaturePubkey, 0)
		transHash     = make([]nhssTransaction.TxHash, 0)
		addressPrefix nhssTransaction.AddressPrefix
	)
	...
	for accountID, keySignatures := range rawTx.Signatures {
		...
		for _, keySignature := range keySignatures {
			...
			signaturePubkey := nhssTransaction.SignaturePubkey{
				Signature: signature,
				Pubkey:    pubkey,
			}
	...
	trx, err := nhssTransaction.DecodeRawTransaction(txBytes, decoder.wm.Config.SupportSegWit)
	if err != nil {
	...
	for _, vin := range trx.Vins {
		...
		txUnlock := nhssTransaction.TxUnlock{
	...
	signedTrans, err := nhssTransaction.InsertSignatureIntoEmptyTransaction(emptyTrans, transHash, txUnlocks, decoder.wm.Config.SupportSegWit)
	if err != nil {
	...
	pass := nhssTransaction.VerifyRawTransaction(signedTrans, txUnlocks, decoder.wm.Config.SupportSegWit, addressPrefix)
	if pass {
...
func (decoder *TransactionDecoder) createBTCRawTransaction(
	...
	var (
		err              error
		vins             = make([]nhssTransaction.Vin, 0)
		vouts            = make([]nhssTransaction.Vout, 0)
		txUnlocks        = make([]nhssTransaction.TxUnlock, 0)
		totalSend        = decimal.New(0, 0)
		destinations     = make([]string, 0)
		accountTotalSent = decimal.Zero
	...
	for _, utxo := range usedUTXO {
		in := nhssTransaction.Vin{utxo.TxID, uint32(utxo.Vout)}
		vins = append(vins, in)

		txUnlock := nhssTransaction.TxUnlock{LockScript: utxo.ScriptPubKey, SigType: nhssTransaction.SigHashAll}
		txUnlocks = append(txUnlocks, txUnlock)
	...
	for to, amount := range to {
		...
		out := nhssTransaction.Vout{to, uint64(amount.IntPart())}
	...
	/////////构建空交易单
	emptyTrans, err := nhssTransaction.CreateEmptyRawTransaction(vins, vouts, lockTime, replaceable, addressPrefix)
	...
	////////构建用于签名的交易单哈希
	transHash, err := nhssTransaction.CreateRawTransactionHashForSig(emptyTrans, txUnlocks, decoder.wm.Config.SupportSegWit, addressPrefix)
	if err != nil {
...
func (decoder *TransactionDecoder) createOmniRawTransaction(
	...
	for _, utxo := range usedUTXO {
		...
		txUnlock := omniTransaction.TxUnlock{LockScript: utxo.ScriptPubKey, SigType: nhssTransaction.SigHashAll}
		txUnlocks = append(txUnlocks, txUnlock)
